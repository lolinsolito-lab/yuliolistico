...
import { Edit2, Eye, EyeOff, X, Loader, Plus, Search, Check, Sparkles, Flame, Crown } from 'lucide-react';
...
const ServicesEditor: React.FC = () => {
    ...
    const groupedServices = {
        [TreatmentType.MANUAL]: filteredServices.filter(s => s.category === TreatmentType.MANUAL),
        [TreatmentType.TOOLS]: filteredServices.filter(s => s.category === TreatmentType.TOOLS),
        [TreatmentType.RITUAL]: filteredServices.filter(s => s.category === TreatmentType.RITUAL),
    };

    const CategorySection = ({ title, icon: Icon, services, colorClass, bgClass }: any) => (
        <div className="space-y-4 mb-12">
            <div className={`flex items-center gap-3 pb-2 border-b ${colorClass} border-opacity-20`}>
                <Icon className={`w-5 h-5 ${colorClass}`} />
                <h3 className={`text-sm font-bold uppercase tracking-widest ${colorClass}`}>{title}</h3>
                <span className="text-xs text-stone-400 bg-stone-100 px-2 py-0.5 rounded-full">{services.length}</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {services.map((service: any) => (
                    <motion.div
                        ...
                        className="group relative bg-white rounded-2xl overflow-hidden shadow-sm border border-stone-100 cursor-pointer flex flex-col"
                        onClick={() => handleEditClick(service)}
                    >
                        {/* Image Header */}
                        <div className="h-48 overflow-hidden relative">
                            <img src={service.image_url} alt={service.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-60" />

                            {/* Status Badge */}
                            <div className="absolute top-3 right-3">
                                <span className={`px-2 py-1 rounded-md text-[10px] uppercase font-bold tracking-wider backdrop-blur-md ${service.active ? 'bg-green-500/20 text-green-100 border border-green-500/30' : 'bg-red-500/20 text-red-100 border border-red-500/30'
                                    }`}>
                            ...
                            <div className="absolute bottom-4 left-4 right-4">
                                <h4 className="text-white font-serif text-lg leading-tight shadow-sm">{service.title}</h4>
                            </div>
                        </div>

                        {/* Content Body */}
                        <div className="p-4 flex flex-col flex-grow">
                            <div className="flex justify-between items-center text-xs text-stone-500 mb-3 border-b border-stone-100 pb-3">
                                <span className="font-mono">{service.duration}</span>
                                <span className={`font-bold ${colorClass.replace('text-', 'text-')}`}>{service.price}</span>
                            </div>

                            <p className="text-xs text-stone-400 line-clamp-2 mb-4 flex-grow italic">
                                "{service.soul_description || service.description}"
                            </p>
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-20 text-stone-400 gap-4">
            ...
            <span className="text-xs uppercase tracking-widest">Generazione Catalogo 3D...</span>
        </div>
    );
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-stone-100 sticky top-0 z-30">
                <div className="relative w-full md:w-96">
                    ...
                    <input
                        type="text"
                        placeholder="Cerca tra i rituali..."
                        value={searchTerm}
            ...
            {/* SECTIONS */}
            <CategorySection
                title="Le Fondamenta (Manuale)"
                icon={Sparkles}
                services={groupedServices[TreatmentType.MANUAL]}
                colorClass="text-[#849b87]"
                bgClass="bg-[#849b87]"
            />

            <CategorySection
                title="La Profondità (Strumenti)"
                icon={Flame}
                services={groupedServices[TreatmentType.TOOLS]}
                colorClass="text-[#c07a60]"
                bgClass="bg-[#c07a60]"
            />

            <CategorySection
                title="La Sovranità (Rituali)"
                icon={Crown}
                services={groupedServices[TreatmentType.RITUAL]}
                colorClass="text-[#d4af37]"
                bgClass="bg-[#1c1917]"
            />

            {/* ── EDIT MODAL (Reused & Enhanced) ── */}
            <AnimatePresence>
